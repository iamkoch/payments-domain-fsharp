...
module private Assert =
    let validName name = 
        notNull ["The name must not be null."] name 
        <* notEmptyString ["The name must not be empty"] name
    let validCount count = 
        validator (fun c -> c > 0) ["The item count must be positive."] count
    let inactive state = 
        validator (fun i -> not i.isActive) ["The item is already deactivated."] state
